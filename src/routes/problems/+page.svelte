...
<div class="min-h-screen bg-gray-900 p-4 font-sans text-gray-400 md:p-8">
	<div class="mx-auto max-w-7xl">
		<div
			class="mb-8 flex flex-col items-start justify-between gap-4 pt-12 sm:flex-row sm:items-center"
		>
			<h1 class="text-4xl font-bold text-gray-100">All Problems</h1>
			<div class="w-full sm:w-1/2 md:w-1/3">
				<input
					bind:value={searchTerm}
					oninput={handleSearch}
					class="ease h-10 w-full rounded-md border border-gray-600 bg-gray-800 px-4 py-2 text-sm text-white shadow-sm transition duration-200 placeholder:text-gray-500 hover:border-gray-500 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
		...
		<div
			class="relative flex h-full w-full flex-col overflow-hidden rounded-lg bg-gray-800 bg-clip-border text-gray-200 shadow-lg"
		>
			<div class="overflow-x-auto">
				<table class="w-full min-w-max table-auto text-left">
					<thead>
						<tr>
							<th class="border-b border-gray-700 bg-gray-700/50 p-3 px-4 sm:p-4">
							...
							<th
								class="hidden border-b border-gray-700 bg-gray-700/50 p-3 px-4 sm:p-4 md:table-cell"
							>
							...
							<th class="border-b border-gray-700 bg-gray-700/50 p-3 px-4 text-center sm:p-4">
					...
					<tbody>
						{#each problemsToDisplay as problem (problem.id)}
							<tr
								...
								class="cursor-pointer border-b border-gray-700 transition-colors hover:bg-gray-700/60 {problem.completed
									? 'bg-[rgba(34,197,94,0.08)]'
									: ''}"
							>
								<td class="p-3 px-4 sm:p-4">
									<div class="flex items-center gap-3">
										...
										{/if}
										<p
											class="block text-sm font-semibold whitespace-nowrap {problem.completed
												? 'text-green-400'
												: 'text-gray-100'}"
										>
											{problem.id}
								...
								<td class="max-w-[200px] truncate p-3 px-4 sm:max-w-xs sm:p-4">
									<p class="text-sm text-gray-300">{problem.problem}</p>
								</td>
								<td class="hidden p-3 px-4 sm:p-4 md:table-cell">
								...
								<td class="p-3 px-4 sm:p-4">
									<span
										...
										class:text-red-400={problem.difficulty === 'Hard'}
									>
										{problem.difficulty}
									</span>
								</td>
			...
			<div
				class="flex flex-col items-center justify-between gap-4 border-t border-gray-700 px-4 py-3 sm:flex-row"
			>
				<div class="text-sm text-gray-400">
					...
					<b>
						...
						{:else}
							0
						{/if}- {Math.min(pageNumber * pageSize, getActiveList().length)}
					</b>
					of
					<b>{getActiveList().length}</b>
				</div>
				...
				<div class="flex space-x-1">
					<button
						...
						class="rounded-md border border-gray-600 bg-gray-700 px-3 py-1.5 text-sm text-gray-300 transition-colors hover:enabled:border-gray-400 hover:enabled:bg-gray-600 disabled:cursor-not-allowed disabled:opacity-50"
					>
						Prev
					</button>
